...
const mockDiplomaCourse = {
  ...
  description:
    "Trains students in laboratory techniques and diagnostic procedures.",
};
...
describe("CourseCard", () => {
  ...
  it("renders all entry requirements", () => {
    renderCard(mockDiplomaCourse);
    expect(
      screen.getByText("O-Level with at least 5 passes"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("A-Level Biology or Chemistry"),
    ).toBeInTheDocument();
  });
